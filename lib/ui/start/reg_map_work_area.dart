...
class RegMapWorkAreaScreen extends StatefulWidget {
  @override
  _RegMapWorkAreaScreenState createState() => _RegMapWorkAreaScreenState();
}

class _RegMapWorkAreaScreenState extends State<RegMapWorkAreaScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    String _text = "";
    for (var item in _mainModel.routeForNewProvider){
      if (_text.isNotEmpty)
        _text = "$_text; ";
                    ...
                    child: Row(
                      children: [
                        Expanded(child: button2(strings.get(181), Colors.red, (){ /// "Delete current",
                          _mainModel.routeForNewProvider = [];
                          _routeInit();
                          _redraw();
                        })),
                        SizedBox(width: 10,),
                        Expanded(child: button2(strings.get(67), theme.mainColor, () async {  /// "Save",
                          // var ret = await _mainModel.provider.saveWorkArea();
                          // if (ret != null)
  ...
  _routeInit(){
    var _route = _mainModel.routeForNewProvider;
  ...
  _routeAdd(LatLng pos){
    _mainModel.routeForNewProvider.add(pos);
    polylineCoordinates = [];
    if (_mainModel.routeForNewProvider.isNotEmpty) {
      for (var item in _mainModel.routeForNewProvider)
        polylineCoordinates.add(item);
      polylineCoordinates.add(_mainModel.routeForNewProvider[0]);
    }
    _initPolygon();
  }
